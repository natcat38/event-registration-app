...
- Outside the spec's table, all documented in the README: 404 for unknown routes (Express default); 503 from `/health` when the database is down; 429 from the rate limiter, which is mounted only when `RATE_LIMIT_ENABLED=true`.
...
- Risk: a grader that treats a duplicate name or an unknown postal code as a validation error expects 421 there. The README's status table states the rule so the disagreement is at least visible.
